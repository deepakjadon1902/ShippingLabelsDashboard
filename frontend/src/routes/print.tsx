...
function PrintPage() {
  ...
  const [printProfileId, setPrintProfileId] = useState("saved");
  ...
  const printProfile = profiles.find((profile) => profile.id === printProfileId) ?? null;
  const printLabels: Label[] = useMemo(() => {
    if (!printProfile) return selectedLabels;
    return selectedLabels.map((label) => ({
      ...label,
      sender_name: printProfile.name || null,
      sender_address: printProfile.address || null,
      sender_phone: printProfile.phone || null,
      sender_website: printProfile.website || null,
      sender_review_url: printProfile.review_url || null,
      sender_profile_id: printProfile.id || null,
    }));
  }, [selectedLabels, printProfile]);
  ...
  const canPrint = printLabels.length > 0;
  ...
  const effectiveLayout: LayoutConfig =
    printLabels.length === 1 && layoutKey === "grid4" ? LAYOUTS.single : LAYOUTS[layoutKey];

  const pages = useMemo(
    () => chunk(printLabels, effectiveLayout.perPage),
    [printLabels, effectiveLayout.perPage],
  );
      ...
      <div className="no-print p-4 md:p-6 max-w-7xl mx-auto space-y-4">
        <div className="flex flex-wrap items-center gap-2 justify-between">
          ...
          <div className="flex flex-wrap gap-2 items-center">
            ...
            <Select value={printProfileId} onValueChange={setPrintProfileId}>
              ...
              <SelectContent>
                <SelectItem value="saved">Use saved label sender</SelectItem>
            ...
            <Button onClick={() => window.print()} disabled={!canPrint}>
              ...
              Print {printLabels.length > 0 ? `(${printLabels.length})` : ""}
            </Button>
          </div>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
          ...
          <Card>
            <CardHeader>
              <CardTitle>
                Preview · {effectiveLayout.title} ({printLabels.length} selected)
              </CardTitle>
            </CardHeader>
            <CardContent>
