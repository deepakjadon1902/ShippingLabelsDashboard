...
function EditLabelPage() {
  ...
  const overrideSender =
    senderChoice !== SENDER_KEEP
      ? (profiles.find((profile) => profile.id === senderChoice) ?? null)
      : null;
  ...
  const previewLabel: Label | null = useMemo(() => {
    ...
    return {
      ...
      sender_profile_id: (overrideSender?.id ?? form.sender_profile_id) || null,
    };
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const senderFields = overrideSender
      ? {
          ...
          sender_profile_id: overrideSender.id || null,
        }
      : {
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="space-y-3">
                    ...
                    <SelectContent>
                      ...
                      {profiles.map((profile) => (
                        <SelectItem key={profile.id} value={profile.id}>
                          Switch to: {profile.name || "Unnamed profile"}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                ...
                <Field label="Status">
                  <Select
                    value={form.status}
                    onValueChange={(v) => set("status", v as LabelStatus)}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
