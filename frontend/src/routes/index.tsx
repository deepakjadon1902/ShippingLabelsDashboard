...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "All your shipping labels in one place. Search, filter and manage every parcel.",
      },
      ...
      <Card>
        <CardHeader className="flex flex-col md:flex-row md:items-center gap-3 md:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            >
              <RefreshCw
                className={`h-4 w-4 mr-1 ${refreshAllMutation.isPending ? "animate-spin" : ""}`}
              />
                ...
                {isLoading ? (
                  ...
                  filtered.map((l) => (
                    <LabelRow
                      ...
                      refreshing={
                        refreshOneMutation.isPending && refreshOneMutation.variables === l.id
                      }
                    />
                  ))
                )}
    ...
    <Card className={accent ? "bg-primary text-primary-foreground" : ""}>
      <CardContent className="p-4">
        <div className="flex items-center justify-between">
          <div
            className={
              "text-sm " + (accent ? "text-primary-foreground/80" : "text-muted-foreground")
            }
          >
            {label}
          </div>
