...
export function ShippingLabel({ label, size = "compact" }: Props) {
  ...
  // Font sizes for the FROM block (kept smaller than the TO section)
  ...
  return (
    ...
    >
      {/* FROM block — top-left, review QR right-of-it. Kept above the main TO row. */}
      {hasSender || hasReviewQr ? (
        <div
          className={
            "flex items-start justify-between pb-2 mb-2 border-b border-black/25 " +
            (isMini ? "gap-2" : isFull ? "gap-4" : "gap-3")
          }
        >
          <div className="flex-1 min-w-0">
            <div className={"font-semibold tracking-[0.2em] uppercase text-black/60 " + fromLabelSize}>
              From
            </div>
            {label.sender_name ? (
            ...
            {label.sender_address ? (
              <div className={"leading-snug text-black/85 break-words whitespace-pre-line " + fromBodySize}>
                {label.sender_address}
              </div>
            ) : null}
            {label.sender_phone ? (
      ...
      {/* Top: receiver on the left, tracking QR neatly on the right */}
